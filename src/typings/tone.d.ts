...
declare namespace Tone {
    ...
    var DrumSynth : {
        new(options?: Object): Tone.DrumSynth;
    };

    interface DrumSynth extends Tone.Instrument {
        triggerAttackRelease(value: any, duration: Tone.Time, time?: Tone.Time, velocity?: number): Tone.DrumSynth;
        dispose(): Tone.DrumSynth;
    }
    ...
    var Limiter: {
        new(threshold: AudioParam|number): Tone.Limiter;
    };
    ...
    var Sequence: {
        new<T>(callback: (time: Tone.Time, note: T) => void, events: Array<T>, subdivision: Tone.Time): Tone.Sequence;
    };
